package com.note.remiads.utils;
...
import com.note.remiads.nativenew.AdsNativeItem;
...
public class RmSave {
    ...
    public static boolean getPay(Context c) {
        return getShare(c).getBoolean("pay_subs", false);
    }

    //Subscription
    public static void putPaySubscription(Context c, boolean isPaid) {
        getShare(c).edit().putBoolean("pay_subs", isPaid).apply();
    }
    ...
    public static boolean isFistOpen(Context c) {
        return getShare(c).getBoolean("is_fist", true);
    }

    public static void setFistOpen(Context c) {
        getShare(c).edit().putBoolean("is_fist", false).apply();
    }
