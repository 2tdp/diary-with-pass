...
public class TextRect {
     ...
     */
    public TextRect(Paint paint, Typeface font, int colorRes, float sizeText, Paint.Align align) {
        metrics = paint.getFontMetricsInt();
        this.paint = paint;
        this.paint.setTypeface(font);
        this.paint.setColor(colorRes);
        this.paint.setTextSize(sizeText);
    ...
    public int prepare(String text, int maxWidth, int maxHeight) {
        ...
        for (; ; ) {
            ...
            for (int o = stop + 1; stop < o && stop > start; ) {
                ...
                if ((lowest >= start && lowest < stop) || bounds.width() > maxWidth) {
                    --stop;
                    ...
                    if (lowest < start || lowest > stop) {
                        ...
                        if (blank > start && (hyphen < start || blank > hyphen)) lowest = blank;
                        else if (hyphen > start) lowest = hyphen;
                    }
    ...
    public void draw(Canvas canvas, int left, int top) {
        ...
        for (int i = 0; i < lines; ++i) {
            ...
            else line = text.substring(starts[i], stops[i]);
